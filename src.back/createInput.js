...
function createInput(key) {
  let form = document.querySelector(
    "regeggs-card.chrome-regeggs-popup regeggs-div.formWrapper"
  );
  ...
  next.addEventListener("click", (e) => {
    ...
    let GI = getGI(key);
    if (!!MY_HIGHLIGHTS[GI]) {
      CURRENT_INDEXES[GI] = nextMatch(
        MY_HIGHLIGHTS[GI].elements,
        CURRENT_INDEXES[GI],
        {
          direction: 1,
          newStyles: {
...
function highlightMe(key, options) {
  ...
  if (options) {
    ...
    ogo.match = ogo.litReg
      ? ogo.match
      : ogo.loose
      ? ogo.match
      : escapeRegExp(ogo.match);
  }
  let GI = getGI(key);
  CUR_INDEX = 0;
  if (GI === -1) {
  ...
  if (
    ogo.match !== "" &&
    DEF_REJECTS.indexOf(ogo.match) === -1 &&
    !!finalRegex
  ) {
    let multiNodeMatchId;
    MY_HIGHLIGHTS[GI] = highlight(
      document.body,
      {
        regex: finalRegex,
